...
import { viewProductForAdminAction } from "../../../actions/sellerProductAction";
import * as moment from "moment";

const AdminConfirmProducts = () => {
	const dispatch = useDispatch();

	useEffect(() => {
		dispatch(viewProductForAdminAction());
	}, [dispatch]);

	const ConfirmSellerProducts = useSelector((state) => state.ConfirmSellerProducts);
	const { loading, product, error } = ConfirmSellerProducts;
	...
	useEffect(() => {
		if (product && product.length > 0) {
			const sortedArr = product.sort((a, b) => {
